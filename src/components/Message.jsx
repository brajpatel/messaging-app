    ...
    return (
        <div className={`flex ${message.user ? 'flex-row-reverse justify-end' : ''} gap-3 mb-4 break-words`}>
            ...
            <div className={`w-full ${message.user ? 'flex flex-col items-end' : ''}`}>
                ...
                <div className={`w-max max-w-[87.5%] md:w-[70%] lg:text-lg px-3 md:px-4 py-2 ${message.user ? 'bg-rose-600 dark:bg-rose-600 text-gray-50' : 'bg-gray-200 dark:bg-gray-800'} rounded`}>
                    {message.message}
                </div>
            </div>
        </div>
